...
{
    [System.Serializable]
    public struct UnitComponentTask //each unit component task is assigned an icon and a task panel category.
    {
        public bool enabled;
        public Sprite icon;
    ...
    public abstract class UnitComponent<E> : MonoBehaviour where E : Entity
    {
        ...
        [SerializeField]
        public UnitComponentTask taskUI = new UnitComponentTask(); //the task that will appear on the task panel when a unit with this component is selected
        ...
        public virtual void Init(GameManager gameMgr, Unit unit)
        {
            ...
            if (this.unit.FactionID != GameManager.PlayerFactionID) //if this unit doesn't belong to the player's local faction
        ...
        public virtual bool Stop()
        {
            ...
            AudioManager.Stop(unit.AudioSourceComp); //stop construction audio
